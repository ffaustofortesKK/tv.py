...
def painel_admin():
    ...
    if solics:
        ...
        for id_firebase, info in (solics.items() if isinstance(solics, dict) else []):
...
def painel_cliente():
    ...
    if col1.button("Entrar"):
        if nome == "ADMIN" and senha == SENHA_ADMIN:
            ...
            # Validação de segurança para o erro de formato
            ...
            else: st.error("Utilizador não aprovado ou dados corrompidos.")
...
if st.session_state.get('nome') == "ADMIN":
    ...
    if token_data and isinstance(token_data.get('expira'), str):
        ...
        else: st.error("Tempo esgotado!"); st.session_state.autenticado = False; st.rerun()
else:
    painel_cliente()
