...
st.markdown("""
    <style>
        ...
        .video-container { display:flex; justify-content:center; align-items:center; height:90vh; }
    </style>
...
if comando == "play" and url_video:
    components.html(f"""
        ...
        <script>
            ...
            // Tenta remover o mudo após o play começar
            v.muted = false;
            v.play().catch(() => {{
                // Se falhar, adicionamos um botão para o utilizador clicar
                document.body.innerHTML += '<button onclick="v.muted=false; v.play(); this.remove();" style="position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); padding:20px; font-size:24px; cursor:pointer;">CLIQUE PARA INICIAR O SOM</button>';
            }});
