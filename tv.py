...
# 3. TELA PRINCIPAL (Fila à esquerda, Vídeo Clipe único à direita)
        ...
        if res_pedidos:
            ...
            for p_id, p in pedidos_lista:
                if not str(p.get('musica', '')).startswith("PEDIDO:"):
                    st.markdown(f"<h3 style='margin: 15px 0; font-size: 1.4rem;'>{contador_exibicao}. <span class='cantor-style'>{str(p.get('cantor')).upper()}</span> ➔ <span class='musica-style'>{str(p.get('musica')).upper()}</span></h3>", unsafe_allow_html=True)
                    contador_exibicao += 1
    ...
    with cl2:
        st.markdown("<h1 style='color:gold; font-size: 2.2rem; margin-bottom: 20px;'>📺 VÍDEO CLIPE (FUNDO)</h1>", unsafe_allow_html=True)
        ...
        if url_clipe and nome_clipe_atual and res_status.get("cantor") == "VÍDEO CLIPE":
            st.markdown(f"<p style='color: #00ff00; font-weight: bold; margin-bottom: 10px; font-size: 1.1rem;'>▶️ Reproduzindo: {nome_clipe_atual}</p>", unsafe_allow_html=True)
            ...
            st.markdown("""
                <div class="video-clipe-box" style="display: flex; align-items: center; justify-content: center; text-align: center; color: #888; padding: 30px;">
